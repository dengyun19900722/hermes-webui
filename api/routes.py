...
def _handle_audit_rotate(handler, parsed):
    """Trigger gzip rotation of old .jsonl files."""
    try:
        from api import audit as _audit
    except ImportError:
        return j(handler, {"error": "audit not available"}, status=503)

    qs = parse_qs(parsed.query)
    days = int(qs.get("days", ["7"])[0])
    result = _audit.rotate_old_logs(age_days=days)
    return j(handler, result)


def _handle_audit_cleanup(handler, parsed):
    """Trigger deletion of compressed audit files older than retention period."""
    try:
        from api import audit as _audit
    except ImportError:
        return j(handler, {"error": "audit not available"}, status=503)
    ...
    qs = parse_qs(parsed.query)
    days_str = qs.get("days", [None])[0]
    days = int(days_str) if days_str else None
    result = _audit.cleanup(days=days)
    return j(handler, result)


def _handle_audit_search(handler, parsed):
    """Search and export audit log entries."""
    try:
        from api import audit as _audit
    except ImportError:
        return j(handler, {"error": "audit not available"}, status=503)
    ...
    limit = min(int(qs.get("limit", ["20"])[0]), 1000)
    page = max(1, int(qs.get("page", ["1"])[0]))
    offset = (page - 1) * limit

    entries = _audit.search(
        since=since,
        until=until,
    ...
    # Total count for pagination (without limit/offset to get full count)
    total = _audit.count(
        since=since,
        until=until,
        category=category or None,
...
def _handle_audit_get(handler, parsed):
    """Fetch a single audit entry by id for detail view."""
    try:
        from api import audit as _audit
    except ImportError:
        return j(handler, {"error": "audit not available"}, status=503)
    ...
    if not entry_id:
        return j(handler, {"error": "missing id"}, status=400)
    entry = _audit.get(entry_id)
    if entry is None:
        return j(handler, {"error": "not found"}, status=404)
    return j(handler, entry)


def _handle_audit_export(handler, parsed):
    ...
    _encoded = _content.encode("utf-8")

    handler.send_response(200)
    handler.send_header("Content-Type", "text/csv; charset=utf-8")
    handler.send_header("Content-Length", str(len(_encoded)))
    handler.send_header(
        "Content-Disposition",
        "attachment; filename*=UTF-8''audit-log.csv",
    )
    handler.send_header("Cache-Control", "no-cache")
    handler.end_headers()
    handler.wfile.write(_encoded)
    return True
